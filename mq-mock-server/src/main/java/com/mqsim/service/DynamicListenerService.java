...
public class DynamicListenerService {
    ...
    public void initializeListeners() {
        logger.info("Initializing dynamic IBM MQ listeners for namespace: {}...", namespaceConfig.getNamespace());
        refreshListeners();
    }
    ...
    public synchronized void refreshListeners() {
        ...
        logger.info("Refreshing IBM MQ listeners for namespace: {}...", namespace);

        stopAllListeners();

        List<QueueConfig> enabledQueues = queueConfigRepository.findByNamespaceAndEnabledTrue(namespace);
        logger.info("Found {} enabled queue configurations for namespace: {}", enabledQueues.size(), namespace);
